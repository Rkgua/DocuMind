...
function Sidebar({ selectedDocs, onDocsChange, onNewChat, onLoadHistory }) {
  ...
  useEffect(() => {
    Promise.all([loadDocuments(), loadHistory()]).finally(() => setLoading(false))
  }, [loadDocuments, loadHistory])

  const handleCheckDoc = useCallback((docId) => {
    setDocuments(prev => {
      ...
      <div className="sidebar-scroll">
        ...
        <div className="sidebar-section">
          ...
          <div className="sidebar-section-body">
                ...
                {uploadProgress.map(item => (
                  <div key={item.id} className="upload-progress-item">
                    ...
                    <span className="file-status">
                      {item.status === 'done' ? '✅ 上传成功' : `${Math.round(item.progress)}%`}
                    </span>
                  </div>
                ))}
              </div>
            ...
            {/* Document List */}
            <div className="sidebar-section-header" style={{ paddingBottom: 4 }}>
              <span>已入库文档</span>
            </div>
            {loading ? (
              <div style={{ padding: '16px 8px', textAlign: 'center', color: 'var(--text-sidebar-secondary)', fontSize: '0.8rem' }}>
        ...
        <div className="sidebar-section" style={{ flex: 1, minHeight: 0 }}>
          ...
          <div className="sidebar-section-body" style={{ overflow: 'hidden' }}>
            <div className="history-list">
              {loading ? (
                <div style={{ padding: '24px 8px', textAlign: 'center', color: 'var(--text-sidebar-secondary)', fontSize: '0.8rem' }}>
                  加载中...
                </div>
              ) : history.length === 0 ? (
                <div style={{
                ...
                history.map(h => (
                  ...
                  >
                    ...
                    <div className="history-info">
                      ...
                      <div className="history-date">{h.created_at || h.date}</div>
                    </div>
                  </div>
                ))
              )}
